# OptiCart - Individual Honors Project
Final Year Project towards creating a chatbot-based web application to demonstrate a full stack, conversational AI, and recommendation system that will assist the process of understanding your needs for meal planning, formulating a shopping list and taking into account the current pricing for individual grocery items for price comparison.

<ins>NOTE:</ins>
This is not the main repository of the project! This repository contains the experimentation of using streamlit and investigating the setup of an LLM model as a reason for learning conversational AI engineering and designing. Please click [here](https://github.com/Ninjangelo/Opticart_FYP) for the latest on the project.
